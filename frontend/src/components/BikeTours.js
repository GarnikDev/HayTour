...
export default function BikeTours() {
    ...
    const totalPrice = (adults * 35) + (kids * 25);

    const stops = ["Dilijan Center", "Parz Lake Trail", "Haghartsin Monastery", "Deer Park Forest", "Old Dilijan"];

    return (
        <Box sx={{ flexGrow: 1, pb: 10 }}>
            <Box sx={tourStyles.header}>
                <Typography variant="h3" sx={{ fontWeight: 800 }}>Dilijan Forest Trails</Typography>
                <Typography variant="subtitle1">Ride Through the "Little Switzerland" of Armenia</Typography>
            </Box>

            <Container sx={{ mt: -6 }}>
                <Paper sx={tourStyles.heroImage("https://images.unsplash.com/photo-1471506480208-91b3a4cc78be?q=80&w=1470")} />
                ...
                <Box sx={{ mt: 8 }}>
                    ...
                    <Typography variant="body1" sx={tourStyles.descriptionText}>
                        Escape into the lush forests of Dilijan on a mountain bike. Perfect for nature lovers looking for adventure and spirit.
                    </Typography>
                </Box>
                    ...
                    <Grid container spacing={4} alignItems="center">
                        <Grid item xs={12} md={6}>
                            <Stack direction="row" spacing={2}>
                                <TextField label="Adults ($35)" type="number" fullWidth sx={{ bgcolor: 'white' }} value={adults} onChange={(e) => setAdults(Math.max(1, parseInt(e.target.value) || 0))} />
                                <TextField label="Children ($25)" type="number" fullWidth sx={{ bgcolor: 'white' }} value={kids} onChange={(e) => setKids(Math.max(0, parseInt(e.target.value) || 0))} />
                            </Stack>
                        </Grid>
